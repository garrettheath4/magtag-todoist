...
def get_display_text_labels():
    ...
    return display, tasks_label, updated_label
...
def fetch_due_today_tasks(requests_session):
    ...
    while next_url:
        ...
        if isinstance(payload, dict):
            ...
            logger.debug("Tasks results: %s", results)
            next_cursor = payload.get("next_cursor", None)
            logger.debug("Next cursor: %s", payload.get("next_cursor", None))
            logger.info("Fetched %d tasks", len(results))
            all_tasks.extend(results)

            # Your request: if there are fewer than 50 tasks in this page, ignore next_cursor.
            if next_cursor:
...
def refresh_tasks(display, tasks_label, updated_label, pixels: NeoPixelDimmer):
    global last_display_refresh
...
def main():
    ...
    global logger
    logger = get_logger("magtag-todoist")
    logger.info("Starting magtag-todoist")

    display, tasks_label, updated_label = get_display_text_labels()
    last_display_refresh = time.monotonic()
    pixels = setup_pixels()
    ...
    refresh_tasks(display, tasks_label, updated_label, pixels)
    ...
    while True:
        ...
        if last_a and not a:
            refresh_tasks(display, tasks_label, updated_label, pixels)  # A: manual refresh
            last_refresh = time.monotonic()
            ...
            refresh_tasks(display, tasks_label, updated_label, pixels)  # D: alternate refresh button
            last_refresh = time.monotonic()
